...
bool flash_cards(string str, int randomNumber, vector<int>& forgotten, bool is_next, bool to_return, int num){
    cout << "█▄▀ ▄▀█ █▄░█ ░░█ █   █▀▀ █░░ ▄▀█ █▀ █░█   █▀▀ ▄▀█ █▀█ █▀▄ █▀\n" << 
            "█░█ █▀█ █░▀█ █▄█ █   █▀░ █▄▄ █▀█ ▄█ █▀█   █▄▄ █▀█ █▀▄ █▄▀ ▄█\n" << std::endl;
...
int main(){
    ...
    while(Nums.size() <= SIZE - 1){
        ...
        if(is_in_Nums != true){
            ...
            if(flash_cards(str, randomNumber, forgotten, is_next, to_return, Nums.size())){
                return 0;
            }
        } 
    ...
    while(true){
        if(forgotten.size() == 0){
            ...
            for(int i = 0; i < y; i++){
                ...
                if(flash_cards(str, randomNumber, forgotten, is_next, to_return, i + 1)){
                    return 0;
                }
                forgotten.pop_back();
            }
